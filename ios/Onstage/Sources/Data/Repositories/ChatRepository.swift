...
final class ChatRepository {
  ...
  func getMessages(conversationId: UUID, limit: Int = 50, offset: Int = 0) async throws -> [Message]
  {
    ...
    return response.messages.map { dto in
      ...
      let agentSteps: [AgentStep]? = dto.agentSteps?.compactMap { stepDTO in
        guard let stepType = AgentStep.StepType(rawValue: stepDTO.type) else { return nil }
        let stepStatus: AgentStep.StepStatus = {
          switch stepDTO.status {
